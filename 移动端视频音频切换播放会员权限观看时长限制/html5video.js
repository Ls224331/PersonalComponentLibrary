            	         ...
            	         var vname=video_url.substring(video_url.lastIndexOf('.'));
            	             ...
            	             { 
            	             	 if (mui_video.paused || mui_video.ended) //当暂停或结束
					            	...
					            	console.log('打算播放')
					            	quanxian()     //调用权限接口，查看是否登录，如果没有那去登录，如果登录查看权限
						   ...
						   mui_video.addEventListener("playing", function()
            	              ...
            	              mui_video.addEventListener("timeupdate", function()
							   { 
								...
								//  控制播放时间，开通会员
								 var time = mui_time_current.innerText
								 var date = time.split(':')
								 console.log(parseInt(date[1]))
								 console.log(user_buy)     //通过权限接口查看当前视频对于当前用户来说是否为收费视频
								 if(user_buy == 2){       //2是收费1是不收费
						      ...
						      mui_video.addEventListener("ended", function()
						       {
								 ...
								 mui_video.pause();
//								 var aa = document.getElementById('end').style.display="block";
						       });
